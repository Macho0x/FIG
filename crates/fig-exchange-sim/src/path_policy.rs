...
/// Paths that only support GET query (not live SUBSCRIBE).
fn is_query_only_path(path: &str) -> bool {
...
pub fn expected_interaction(path: &str) -> Interaction {
    ...
    if is_query_only_path(path) || parse_order_book_path(path).is_some() {
        return Interaction::RequestGet;
    }
